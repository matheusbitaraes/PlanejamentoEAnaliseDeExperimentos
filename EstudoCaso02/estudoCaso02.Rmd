...
Tendo as premissas validadas, foi realizado o teste one way ANOVA
```{r, include=TRUE}
# transformação dos dados
perc <- c(data_a1, data_a2, data_a3, data_a4, data_a5)
groups <- c(rep(1, length(data_a1)), rep(2, length(data_a2)), rep(3, length(data_a3)), rep(4, length(data_a4)), rep(5, length(data_a5)))
data_t = data.frame(perc, groups)

# ANOVA
model <- aov(perc ~ group, data = data_t)
summary(model)
```
