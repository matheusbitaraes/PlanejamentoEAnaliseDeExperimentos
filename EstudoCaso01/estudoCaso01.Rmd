...
## Descrição do problema
   ...
   Neste estudo, deseja-se comparar o Índice de Massa Corpórea (IMC) médio de duas populações de alunos da pós-graduação da Engenharia Elétrica (PPGEE) da UFMG do segundo semestre de 2016 e do segundo semestre de 2017.
...
1) Há evidências estatísticas de que a média ($\mu$) do IMC dos alunos do PPGEE-UFMG de 2/2016 é diferente da média do IMC dos alunos do PPGEE-UFMG de 2/2017? (subpopulação masculina)

2) Há evidências estatísticas  de que a mediana (med) do IMC das alunas do PPGEE-UFMG de 2/2016 é diferente da mediana do IMC das alunas do PPGEE-UFMG de 2/2017? (subpopulação feminina)

**Isso só faz sentido estar escrito se for justificado depois, que era o que eu tinha feito com aquele parágrafo depois dos boxplots**
...
**Esse parágrafo que ficava depois do boxplot e antes das análises**
...
```
Através da análise gráfica é possível perceber alguns traços que serão avaliados por testes estatísticos posteriormente. Nota-se que o IMC das subpopulações femininas são bastante distintos de um ano para outro, inclusive o máximo do ano de 2017 é muito próximo do mínimo de 2016 o que sugere uma diferença grande entre as populações. Em contra partida, as subpopulações masculinas apresentam uma distribuição dos valores do IMC mais semelhantes, o que sugere pouca distinção entre elas. 
...
A intenção inicial é realizar o Teste T para a comparação destas amostras. Para isso, precisamos validar que as amostras apresentam uma distribuição normal e que suas variâncias possam ser consideradas iguais (homocedasticidade).
...
Analisando o p-valor, temos que p-valor > 0,05 ($\alpha$) e, portanto, não há evidência estatística para que seja rejeitada a hipótese nula e, portanto, será considerada a homocedasticidade entre as subpopulações.
...
Apesar de não termos evidências para assumirmos que a distribuição de $2017$ é normal, podemos utilizar este valor de potência (considerando as distribuições normais) de 0,456 como uma aproximação da real potência do teste, para que ao menos tenhamos um direcionamento.

**Não termos evidências para assumirmos a ditribuição normal acho que não está certo... Acho que " Apesar de ter-se evidências pelo teste de Shapiro-Wilk para rejeitar-se a hipótese nula da distribuição normal de 2017**
...
Antes de dar início as conclusões para cada subpopulação, é importante retomar algumas definições que foram feitas neste estudo e que são comuns para ambas subpopulações. Foi definido, portanto, um Nivel de Significância ($\alpha$) de 0,05 e uma Potência do Teste ($\pi$) de 0,80. Além disso, foi efetuado um tratamento nos dados das amostras para ficarem com um formato uniforme.
...
Posteriormente, foi utilizado o Teste de Cohen que nos permitiu concluir um tamanho de efeito de  d = 0,1665. Por último, foi utilizada a função `power.t.test` que retornou o valor de 3,4% para a Potência de Teste. Destes dados, foi possível inferir que para a Potência de Teste alcançar o valor de 80% conforme definido, seriam necessárias 8626 amostras.
...
Com base nas amostras fornecidas, nos estudos realizados e nos testes
apresentados neste documento foi possível concluir que as amostras das subpopulações femininas não apresentaram distribuição normal através do teste de Shapiro-Wilk, o que impossibilita de prosseguir com o Teste T.
...
Como mencionado anteriormente, para as subpopulações femininas foi necessário prosseguir com testes não paramétricos, inclusive no cálculo da Potência de Teste. Foi escolhida a abordagem de Shieh e este método requer uma definição da distribuição das amostras para o cálculo. Neste caso, embora não tenhamos evidências para assumir que a distribuição de 2017 é normal, foi necessário considerar desta maneira para que o teste retornasse um valor conclusivo. Assim sendo, sabemos que o resultado de 0,45 não representa de fato o valor para as amostras em estudo, porém ele funciona como uma aproximação do valor real para os dados em questão. É importante mencionar que  a ausência de normalidade da distribuição feminina do segundo semestre de 2017 impossibilitou também a aplicação da função `power.t.test`.

###  c. Discussões gerais e melhorias

Por fim, é importante ressaltar que a quantidade de amostras fornecida de um modo geral foi baixa. Para as subpopulações masculinas, nas quais os dados seguiram distribuição normal, ainda foi possível determinar a Potência de Teste, mas notou-se um valor muito baixo, sendo necessário aumentar significativamente o número de amostras caso deseje-se uma Potência de Teste maior. Para as subpopulações femininas, as amostras não seguiram distribuição normal e, portanto, foi necessário empregar uma análise não paramétrica. Este tipo de análise tem menos poder e isto somado ao baixo número de amostras impossibilitou de se chegar a um valor da Potência de Teste para esta subpopulação. Ainda sim, com o objetivo de encontrar algum valor que se relacionasse e se aproximasse da Potência de Teste para as amostras fornecidas, empregou-se um teste não paramétrico e assumiu-se distribuição normal.

Tendo este cenário em vista, uma forma de melhorar este experimento seria o aumento do número de amostras para todas as subpopulações. Ressaltamos ainda a dificuldade principalmente em relação às amostras da população feminina de 2017 que, além de muito escassas, não apresentaram distruibuição normal o que impossibilitou a realização de alguns testes.
