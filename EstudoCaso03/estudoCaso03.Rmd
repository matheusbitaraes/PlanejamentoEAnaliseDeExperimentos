...
Dentre estes algoritmos, um método que tem sido bastante utilizado nos últimos anos é conhecido como Evolução Diferencial - ED (do inglês, differential evolution - ED) \cite{Storn1997}.
...
Para a execuçao dos experimentos, foram utilizados os pacotes ExpDE e smoof. A classe de funções de interesse para este teste é composta por funções Rosenbrock de dimensão entre 2 e 150.
...
**alterei este parágrafo porque era cpóia ue a menina me enviou. Ai evitamos qualquer stress**
...
Execução piloto com a escolha inicial de 10 execuções e com os 34 blocos obtidos anteriormente.
...
**Aqui é minimizar o limite de precisao?**
...
Visto que os dados não possuem a média dos grupos com uma distribuição homogênea, foi plotado um gráfico com o rendimento dos dois algoritmos para uma análise visual.
...
Dado o gráfico anterior, é possível observar que a solução ótima do Algoritmo 1 é maior que a solução ótima do Algoritmo 2 para todas as dimensões escolhidas, como pode ser confirmado analisando as colunas mean.instance1 e mean.instance2 da tabela data. Portanto, conclui-se, sem a necessidade de novos testes, que o Algoritmo 2 possui melhor solução média em relação ao primeiro algoritmo.

**Achei melhor não entrar no mérito da existência ou não de testes... Não perguntamos o professor e se a gente fizer conclusões erradas ficará pior... ai pensei em concluir essa informação no visualmente e pronto. o que acham?**

## Discussões e Conclusões

O estudo teve como objetivo definir entre dois algoritmos de otimização, qual possui o melhor desempenho para uma dada classe de problemas de otimização. Os dois algoritmos a serem comparados utilizavam o método conhecido como evolução diferencial. Para a análise de desempenho utilizou-se a classe de funções Rosenbrock de dimensão entre 2 e 150. Neste caso, o algoritmo que apresentar os menores valores para suas melhores soluções quando aplicadas à função de Rosenbrock é considerado o algoritmo de melhor desempenho.
...
Observando-se o gráfico **referenciar o grafico aqui** e as considerações realizadas na sua descrição **(me refiro a analise do diego)**, concluiu-se que o algoritmo 2 apresentou melhor desempenho na otimização da função de Rosenbrock. 
