...
```{r, include=TRUE}
#Execução do Shapiro Wilk
shapiro.test(data$mean.instance1)
shapiro.test(data$mean.instance2)
```
**Eu fiz o teste já na coluna meaninstance 1 e 2**


**Matheus, o trabalho do seu amigo dá um pvalor menor e outro maior e ele assumiu que é normal... ai nao sei hahahah**
...
datatable1 = c(data$mean.instance1)
datatable2 = c(data$mean.instance2)

friedman.test(datatable)
```

Como interpretação do teste, temos que se o p-valor < 0.05 ($\alpha$), deve-se rejeitar a hipótese nula, ou seja, as médias dos grupos de amostras não são homogêneas, caso contrário, não há evidências para se rejeitar a hipótese nula. Portanto, analisando os resultados dos testes dispostos acima, onde o p-valor é maior que 0.05, pode-se considerar que a média das populações são diferentes.

**Problemas: se considerarmos que o shapiro wilk deu que nao sao normais, precisamos fazer um teste diferente do ANOVA. Foi o que eu fiz com o teste de Friedman. No entanto, como esperado, deu que as médias são diferentes. No entanto, como não consideramos normais, não podemos usar o teste de Tukey... Mas não achei outro que poderia ser usado... na aula o professor falou que o anova é robusto a pesquenas violações, mas não sei interpretar se podemos ignorar o shapior e fazar o anova...**

**Portanto, precisamos decidir o que será feito ou se a gente encontra um teste nao parametrico para usar depois que o teste de Friedman teve a hipótese nula rejeitada**
